...
from ..aio.tracker import AnyIOSendTracker
...
class AnyIOH2Stream(AsyncHttp2Stream):
    ...
    _headers_received: Optional[anyio.Event]
    _received_trailers: Optional[HeadersType]
    _trailers_received: Optional[anyio.Event]
    _received_data: StreamDataBuffer

    _event_dispatcher: _EventDispatcher

    def __init__(self, connection: "AnyIOH2Connection", stream_id: int) -> None:
    ...
    async def _send_guard(self):
        """Context manager to guard stream operations against exceptions."""
        try:
            yield
    ...
    async def send_headers(self, headers: HeadersType, end_stream: bool = False) -> None:
        ...
        async with self._send_guard():
            if self._id <= 0:
                ...
                tracker = AnyIOSendTracker(lambda: self._atomic_initialize(headers, end_stream))
            else:
                # Stream already initialized, send headers frame
                tracker = AnyIOSendTracker(lambda: self._atomic_send_headers(headers, end_stream))
    ...
    async def send_data(self, data: bytes, end_stream: bool = False) -> None:
        ...
        async with self._send_guard():
            ...
            while True:
                tracker: AnyIOSendTracker[int] = AnyIOSendTracker(lambda: self._atomic_send_data(data_view, end_stream))
                await self._conn.send(tracker)
    ...
    async def reset(self, error_code: Union[Http2ErrorCode, int] = Http2ErrorCode.NO_ERROR) -> None:
        ...
        async with self._send_guard():
            tracker = AnyIOSendTracker(lambda: self._atomic_reset(error_code))
            await self._conn.send(tracker)
            await tracker.result()
    ...
    async def ack_data(self, ack_size: int) -> None:
        """Acknowledge the received data"""
        if ack_size <= 0:
            return
        tracker = AnyIOSendTracker(lambda: self._atomic_ack_data(ack_size), no_wait=True)
        await self._conn.send(tracker)

    async def receive_data(self, max_bytes: int = -1) -> bytes:
        """Receive data from the stream, up to max_bytes."""
        try:
    ...
    async def receive_data_exactly(self, exact_bytes: int) -> bytes:
        """Receive exactly `exact_bytes` bytes from the stream."""
        if exact_bytes < 0:
    ...
    def _handle_data_received(self, event: h2_events.DataReceived) -> None:
        ...
        try:
            self._received_data.append_nowait(event.data or b"", event.flow_controlled_length or 0)
